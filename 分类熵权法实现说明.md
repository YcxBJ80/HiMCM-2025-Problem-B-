...
1. **第一阶段：分类内熵权计算**
   - 将指标按照业务逻辑分为7个类别：能源消费类、能源价格类、碳排放类、经济人口类、气候环境类、城市水资源类、航空出行类（废弃物管理类已预留，等待提供州级数据后启用）
...
代码中定义了7个分类及其关键词（其中废弃物管理类会在具备州级数据后启用）：
...
- **城市水资源类**：来自《副本环境数据汇总》中城市用水表，聚合各州公共供水、灌溉、工业等用水总量
- **航空出行类**：来自FAA机场客运数据表，聚合各州登机旅客规模、机场数量及增速
- **废弃物管理类**：暂未启用，待提供州级废弃物流向数据后纳入
